game.StarterGui:SetCore("SendNotification",{Title="Play NRR";Text="You should play NRR, Nations Roleplay : Remastered has no hackers nor does it have FP it has troop system and is really fun";Duration=10})
...
if not (isfolder and makefolder and readfile and writefile and isfile) then
    cansave = false
    saves = loadstring(game:HttpGet("https://pastebin.com/raw/xxHs3sn3"))()
end
if cansave then
    if not isfolder("mapgame") then makefolder("mapgame") end
    local S = loadstring(game:HttpGet("https://pastebin.com/raw/xxHs3sn3"))()
    if not isfolder("mapgame/savesWW2") then
        makefolder("mapgame/savesWW2")
        for i,v in next, S do writefile("mapgame/savesWW2/"..i..".lua",v) end
    end
end
...
m.Button1Down:Connect(function()
    local t = m.Target
    if t.Name ~= "Province" and t.Name ~= "Union" then return end
...
spawn(function()
    while task.wait(.25) do
        ...
        for i,v in next, provinces:GetChildren() do
            if v:IsA("Model") or v.Name == "Ocean" or ((v.Name ~= "Province" and v.Name ~= "Union") and v.Name ~= "Union") then continue end
...
for i,v in next,provinces:GetDescendants() do
    if v:IsA("BasePart") and (v.Name == "Province" or v.Name == "Union") then
        local p1,p2 = get(v)
    ...
    if t then
        ...
        for i,v in next, provinces:GetChildren() do
            if v:IsA("BasePart") then
                ...
                if (v.Name == "Province" or v.Name == "Union") and not checkcol(v.Color, c) then
...
function save(name)
    ...
    s = s.."}"
    if cansave then
        writefile("mapgame/savesWW2/"..name..".lua",s)
    else
        saves[name] = s
    end
end
...
function loadmap(name)
    if cansave then
        if not isfile("mapgame/savesWW2/"..name..".lua") then return false end
    else
        if saves[name] == nil then return false end
    end
    local T
    if cansave then
        T = loadstring(readfile("mapgame/savesWW2/"..name..".lua"))()
    else
...
function replace(col1,col2)
    ...
    if t then
        ...
        for i,v in next, provinces:GetChildren() do
            if v:IsA("BasePart") then
                if (v.Name == "Province" or v.Name == "Union") and v.BrickColor == c1 then
...
function CHAT(msg)
    ...
    elseif cmd == "del" then
        if cansave then
            delfile("mapgame/savesWW2/"..a[1]..".lua")
        else
            saves[a[1]] = nil
        end
    ...
    elseif cmd == "select" then
        if #a[1] > 0 then
            if a[1] == "all" then
                for i,v in next, provinces:GetChildren() do
                    if (v.Name ~= "Province" and v.Name ~= "Union") then continue end
